...
class ComparisonAgent(FeatureProductAgent):
    ...
    async def _get_final_comparison_answer_feature_level(self, query, data):
        ...
        try:
            system_msg = (
                "تو یک متخصص مقایسه محصولات هستی. "
                "بر اساس داده‌های ارائه شده، یک پاسخ کوتاه برای مقایسه دو محصول ارائه بده. "
                "پاسخ باید شامل نتیجه‌گیری واضح بر اساس داده‌های ارائه شده باشد."
            )
            ...
            لطفاً یک پاسخ کامل و مفصل برای مقایسه این دو محصول ارائه بده.
            """
    ...
    async def _feature_level_comparison(self, query, product_random_key_1, product_name_1, product_random_key_2, product_name_2):
        ...
        final_answer = await self._get_final_comparison_answer_feature_level(query, data)
        return final_answer
    ...
    async def process_query(self, query: str) -> Response:
        ...
        if comparison_type == "feature_level":
            final_answer = await self._feature_level_comparison(query, product_random_key_1, product_name_1, product_random_key_2, product_name_2)
        elif comparison_type == "shop_level":
            final_answer =  await self._shop_level_comparison(query,product_random_key_1, product_name_1, product_random_key_2, product_name_2)
        elif comparison_type == "warranty_level":
            final_answer =  await self._warranty_level_comparison(query,product_random_key_1, product_name_1, product_random_key_2, product_name_2)
        elif comparison_type == "city_level":
            final_answer =  await self._city_level_comparison(query,product_random_key_1, product_name_1, product_random_key_2, product_name_2)
