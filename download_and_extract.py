...
def extract_tar_gz(tar_gz_file, backup_folder):
    """
    Extract tar.gz file to backup folder
    
    Args:
        tar_gz_file (str): Path to the tar.gz file
        backup_folder (str): Name of the backup folder to create and extract to
    ...
    backup_dir = os.path.join(project_root, backup_folder)
    ...
    try:
        with tarfile.open(tar_gz_file, "r:gz") as tar:
            ...
            print(f"📦 Extracted {tar_gz_file} to {backup_dir}")
        return backup_dir
    except Exception as e:
